...
public class DataSeeder(MyDbContext ctx, IConfiguration config)
{
    public async Task Initialize()
    {
        ...
        try
        {
            ...
            logger.LogInformation("Default users seeding completed successfully");
        }
        catch (Exception ex)
        {
            logger.LogWarning($"Database seeding skipped: {ex.Message}. Make sure PostgreSQL is running and accessible.");
        }
    }
}